...
-- ... (GetGroupedSpells, EvaluateConditions, ResolveStep same as before) ...
...
function Rot:AddStep(name, criteria)
    table.insert(NS.db.steps, { name = name, criteria = criteria, conditions = {} })
end

function Rot:UpdateStep(index, name, criteria)
...
function Rot:RemoveStep(index)
    table.remove(NS.db.steps, index)
end
...
function Rot:MoveStep(index, direction)
    ...
    elseif direction == 1 and index < #list then
        list[index], list[index+1] = list[index+1], list[index]
    end
end
...
function Rot:AddCondition(stepIndex, data)
    ...
    if step then
        ...
        local newCond = {}
        for k,v in pairs(data) do newCond[k] = v end
        table.insert(step.conditions, newCond)
    end
end

function Rot:UpdateCondition(stepIndex, condIndex, data)
    local step = NS.db.steps[stepIndex]
    if step and step.conditions and step.conditions[condIndex] then
        local newCond = {}
        for k,v in pairs(data) do newCond[k] = v end
        step.conditions[condIndex] = newCond
    end
end
...
function Rot:MoveCondition(stepIndex, condIndex, direction)
    ...
    if direction == -1 and condIndex > 1 then
        list[condIndex], list[condIndex-1] = list[condIndex-1], list[condIndex]
    elseif direction == 1 and condIndex < #list then
        list[condIndex], list[condIndex+1] = list[condIndex+1], list[condIndex]
    end
end
...
f:SetScript("OnUpdate", function(self, elapsed)
    ...
    for _, step in ipairs(NS.db.steps) do
        ...
        if bestSpell then
            bestSlot = bestSpell.SLOT
            break 
        end
    end
    if JamboSpells and JamboSpells.SetActionSlot then
        JamboSpells:SetActionSlot(bestSlot)
    end
end)
