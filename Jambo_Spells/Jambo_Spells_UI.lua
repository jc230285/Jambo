...
function J:InitUI()
    ...
    f:SetScript("OnDragStart", f.StartMoving); f:SetScript("OnDragStop", f.StopMovingOrSizing); f:SetToplevel(true)
    ...
    f:SetBackdropColor(0.1, 0.1, 0.1, 0.95); f:SetBackdropBorderColor(0, 0, 0, 1)
    
    f.title = f:CreateFontString(nil, "OVERLAY", "GameFontNormal")
    f.title:SetPoint("TOP", 0, -8); f.title:SetText("Jambo Spells V3")
    ...
    refresh:SetSize(80, 22); refresh:SetPoint("TOPLEFT", 10, -10); refresh:SetText("Refresh")
    ...
    export:SetSize(80, 22); export:SetPoint("LEFT", refresh, "RIGHT", 5, 0); export:SetText("Export JSON")
    export:SetScript("OnClick", function() if J.ExportData then J:ExportData() end end)

    -- HEADERS
    local headers = {
        {name="[Slot] [ID] Name (Rank)", width=450, key="NAME"},
    ...
    for _, h in ipairs(headers) do
        ...
        btn:SetSize(h.width, 20); btn:SetPoint("TOPLEFT", x, -40); btn:SetText(h.name)
    ...
    scroll:SetPoint("TOPLEFT", 10, -70); scroll:SetPoint("BOTTOMRIGHT", -30, 10)
    ...
    for i = 1, MAX_ROWS do
        ...
        row:SetHeight(ROW_HEIGHT); row:SetPoint("TOPLEFT", 10, -70 - (i-1)*ROW_HEIGHT); row:SetPoint("RIGHT", -30, 0)
        
        row.icon = row:CreateTexture(nil, "ARTWORK"); row.icon:SetSize(ROW_HEIGHT-2, ROW_HEIGHT-2); row.icon:SetPoint("LEFT", 2, 0)
        row.text = row:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall")
        row.text:SetPoint("LEFT", row.icon, "RIGHT", 5, 0); row.text:SetWidth(420); row.text:SetJustifyH("LEFT")
        
        row.stats = {}
        local statX = 460
        for j=1, 6 do 
            local fs = row:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall")
            fs:SetPoint("LEFT", statX, 0); fs:SetWidth(50); fs:SetJustifyH("RIGHT")
            table.insert(row.stats, fs); statX = statX + 52
        end
        
        row:SetScript("OnEnter", function(self)
            if not self.data then return end
            GameTooltip:SetOwner(self, "ANCHOR_RIGHT"); GameTooltip:AddLine(self.data.NAME)
...
function J:RefreshUI()
    ...
    for i = 1, MAX_ROWS do
        local row = J.rows[i]; local idx = offset + i
        if idx <= #data then
            local info = data[idx]; row.data = info; row:Show()
            ...
            local slotStr = (info.SLOT > 0) and string.format("|cff00ff00[%d]|r ", info.SLOT) or "|cff888888[-]|r "
            local txt = string.format("%s%s|cffaaaaaa[%d]|r %s |cffffff00(R%d L%d)|r", 
                slotStr, color, info.ID, info.NAME, info.RANK, info.LEVEL)
