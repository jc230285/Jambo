...
const screen = {
  grabRegion: async (region) => {
    const cap = robot.screen.capture(region.x, region.y, region.width, region.height);
    return {
      toRGB: () => {
        const rgb = [];
        for (let i = 0; i < cap.image.length; i += 4) {
          rgb.push(cap.image[i + 2], cap.image[i + 1], cap.image[i]); // BGR to RGB
        }
        return rgb;
      }
    };
