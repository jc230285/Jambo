...
If you want to use the Python GUI helpers included in this repo (like `autofish.py` and `analyze.py`) you'll need to install the Python dependencies first.

1) Open PowerShell and install dependencies using the same Python interpreter you use to run the script. Example (adjust the path to your python.exe if needed):

```powershell
& C:/Users/jkkec/AppData/Local/Programs/Python/Python313/python.exe -m pip install -r requirements.txt
```

2) If you don't have `pip` or get permission errors, run PowerShell as Administrator or invoke the python executable from the Install location. If installation succeeds, verify by running:

```powershell
& C:/Users/jkkec/AppData/Local/Programs/Python/Python313/python.exe -c "import mss, cv2, numpy, PIL, win32api; print('OK')"
```

3) Then you can run the GUI script the same way you did before:

```powershell
& C:/Users/jkkec/AppData/Local/Programs/Python/Python313/python.exe "c:/Program Files (x86)/World of Warcraft/_classic_era_/Interface/AddOns/Jambo_Python_V3/fish/autofish.py"
```

Alternatively, there is a convenience batch script `run_autofish.bat` in the repository which attempts to install dependencies from `requirements.txt` and then runs the script. Example usage (PowerShell):

```powershell
& "c:/Program Files (x86)/World of Warcraft/_classic_era_/Interface/AddOns/Jambo_Python_V3/fish/run_autofish.bat" "C:/Users/jkkec/AppData/Local/Programs/Python/Python313/python.exe"
```

If you use `python` on your PATH, you can omit the Python path argument:

```powershell
& "c:/Program Files (x86)/World of Warcraft/_classic_era_/Interface/AddOns/Jambo_Python_V3/fish/run_autofish.bat"

You can also use an elevated PowerShell runner (`run_autofish.ps1`) that will request administrator permissions, install dependencies, and run the script for you. Example usage:

```powershell
Start-Process powershell -ArgumentList "-ExecutionPolicy Bypass -File \"c:/Program Files (x86)/World of Warcraft/_classic_era_/Interface/AddOns/Jambo_Python_V3/fish/run_autofish.ps1\" \"C:/Users/jkkec/AppData/Local/Programs/Python/Python313/python.exe\"" -Verb RunAs
```

Or simply run the script directly (it will attempt to elevate):

```powershell
cd "c:/Program Files (x86)/World of Warcraft/_classic_era_/Interface/AddOns/Jambo_Python_V3/fish"
powershell -ExecutionPolicy Bypass -File .\run_autofish.ps1 "C:/Users/jkkec/AppData/Local/Programs/Python/Python313/python.exe"
```

The PowerShell runner will:
- Request elevation (Administrator) if required.
- Install/upgrade pip and install dependencies from `requirements.txt`.
- Verify imports for the main packages used by the script.
- Run `autofish.py` and display the exit code.

## UI Overlay and Timeout Display

The GUI now stays on top of other windows by default. When the bot finds a bobber, a small overlay label appears on the game screen at the bobber location showing a confidence percentage and a label "Bobber". The bot also shows a countdown in the GUI for the active `reel_timeout` (default 30 seconds).

If you prefer the overlay to be disabled, minimize or close the program window (or stop the bot), which will hide the overlay.

```

If you still get a ModuleNotFoundError for 'mss' or any other package, double-check that the python.exe shown in the command is the same one you used to do the `pip install` step above.
...
The bot will stop watching the bobber after a configurable amount of time if no splash is detected. The default `reel_timeout` value is 30 seconds. You can adjust this value from the bot GUI or in the `fishing_config.json` file under the `reel_timeout` field.
...
```
/script UseAction(your lures key);t
