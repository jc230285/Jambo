...
function Tabs:AssignGuide(guidetitle,step,shared,mode)
	...
	ZGV.IconSets.TabsIcons[self.guide.type]:AssignToTexture(self.Icon)
...
function Tabs:RemoveTab()
	...
	if self.guide and self.guide.headerdata then
		if self.guide.headerdata.shared then -- if shared tab is removed, stop share slaving
			ZGV:ScheduleTimer(function() ZGV.Sync:Deactivate() end,0)
		end
		if self.guide.headerdata.poiloader then -- we were tracking poi, no longer
			ZGV.db.char.activepoi = nil
			ZGV.Poi.DataProvider:RefreshAllData(true)
		end
	end
...
-- check if any tab has given guide
function Tabs:IsGuideTabbed(guide)
	for i,tab in ipairs(Tabs.Pool) do
		if tab.guide==guide then
			return true
		end
...
-- check inactive tabs if any was last focused on a step that is completed
function Tabs:CheckForStepCompletion()
	for i,tab in ipairs(Tabs.Pool) do
		if tab.guide then
			local guide = tab.guide
			local step = guide.steps and guide.steps[tab.step]
			if step then
				if step:IsComplete() and not tab.isActive then
					tab.Text:SetTextColor(0,1,0,1)
				else
					tab.Text:SetTextColor(unpack(SkinData("TabsTextColor")))
				end
			end
