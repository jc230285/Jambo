...
function GearFinder:IsValidDungeon(dungeon, instanceId, diff)
	...
	if dungeon.phase and not ZGV.Dungeons.Phases[dungeon.phase] then return false, false, ident, 0, false,false, "phase inactive" end

	if (ZGV.GetChromieTime()>0 and ZGV.GetChromieTime()~=dungeon.expansionLevel) then return false, false, ident, 0, false,false, "wrong chromie time" end

	local maxScaleLevel = (ZGV.GetChromieTime()==dungeon.expansionLevel) and 50 or dungeon.maxScaleLevel

	-- handle permanent rejects
	if dungeon.max_level and dungeon.max_level<ItemScore.playerlevel then return false, false, ident, 0, false,false, "instance outleveled" end
	...
	if dungeon.difficulty then
		if dungeon.difficulty==23 then
			if not (ZGV.db.profile["gear_23"] or ZGV.db.profile["gear_8"]) then return false, false, ident, 0, false,false, "instance filtered out "..dungeon.difficulty end
		else
			if not ZGV.db.profile["gear_"..dungeon.difficulty] then return false, false, ident, 0, false,false, "instance filtered out "..dungeon.difficulty end
		end
	end

	if dungeon.buff then
		if not ZGV.Parser.ConditionEnv.hasbuff("spell:"..dungeon.buff) then return false, false, ident, 0, false,false, "no timewalking buff "..dungeon.buff end
		maxScaleLevel = ItemScore.playerlevel -- they are valid even if blizz lies to us
	end
	...
	if dungeon.difficulty==17 then
		...
		if not isAvailableToPlayer then
			...
			if ( lockedReason == 1029 or lockedReason == 1030 or lockedReason == 1031 ) then --WRONG_TIME_RANGE, WRONG_TIME, WRONG_WORLD_STATE_EXPRESSION
				return false, false, ident, 0, false,false, "not available yet"
			end
		end
	end

	local mythic = dungeon.difficulty==23 
	local mythicplus = mythic -- dungeon.difficulty==8  -- mythic and dungeon.expansionLevel==ZGV.Dungeons.CurrentExpansion
	...
	if ZGV.IsRetail and not ZGV.db.profile.ga_removeilvl and dungeon.min_ilevel and dungeon.min_ilevel > ItemScore.playeritemlvl then return false, true, ident, maxScaleLevel, mythic,mythicplus, "need higher item level "..dungeon.min_ilevel end
...
function GearFinder:ScoreDungeonItems()
	...
	GearFinder.debugdungeons = {valid={},invalid={},status={ok={}}}
	local debugdungeons = GearFinder.debugdungeons
	...
	for dungeon,dungeondata in pairs(ZGV.ItemScore.Items) do
		...
		if valid then
			...
			debugdungeons.status.ok[dungeon] = debugdungeons.valid[dungeon]
		else
			debugdungeons.invalid[dungeon] = {valid, future, ident, maxscale, mythic, mythicplus, comment}
			debugdungeons.status[comment or "?"] = debugdungeons.status[comment or "?"] or {}
			debugdungeons.status[comment or "?"][dungeon] = debugdungeons.invalid[dungeon]
		end
		...
		local mythicplus = mythic and ZGV.db.profile.gear_8 
