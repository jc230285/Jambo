...
function LibRover_Node:CanFlyTo(dest,debug)
	...
	if (m==MAPENUM_VALE_NEW or dest_m==MAPENUM_VALE_NEW) and ZGV.InPhase("oldvale") then return false,debug and "no flying between timetravel zones" end
	if (MAPENUM_VALE_OLD[dest_m] or MAPENUM_VALE_OLD[m]) and not ZGV.InPhase("oldvale") then return false,debug and "no flying between timetravel zones" end
